...
def create_github_app_qs(domain="data-dev.lsst.cloud"):
    ...
    """
    parameters = [
        ("name", f"Semaphore ({domain})"),
        (
            "description",
            "Semaphore is the user notification service for the Rubin "
            f"Science Platform ({domain}). An instance of the Semaphore app "
            "is associated with each "
            "[RSP environment]("
            "https://phalanx.lsst.io/environments/index.html).",
        ),
        ("url", "https://semaphore.lsst.io"),
        ("public", "false"),
        ("webhook_active", "true"),
        ("webhook_url", f"https://{domain}/semaphore/github/webhook"),
        ("events[]", "push"),
        ("events[]", "check_run"),
        ("contents", "read"),
        ("checks", "write"),
